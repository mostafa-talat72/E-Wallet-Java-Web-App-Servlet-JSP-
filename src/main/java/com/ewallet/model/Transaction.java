...
/**
 * Maps to the {@code transactions} table. Represents a money movement between
 * two accounts ({@code fromAccountId} -> {@code toAccountId}) with an amount,
 * optional fees, a type and a status; the reference number uniquely identifies
 * the operation for users and support.
 */
public class Transaction {

    // Account and classification references
    ...
    /**
     * Creates a new transaction for persistence (INSERT): references the
     * source/target accounts, the type and the initial status; timestamps are
    ...
    /**
     * Full constructor used when a transaction row is read back from the
     * database, including the lifecycle timestamps.
     */
    ...
    // Getters & Setters — trivial accessors, kept on single lines
